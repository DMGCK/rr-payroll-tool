...
const routesFieldContainerElement = document.getElementById('routesFieldContainer');
...
function updateTotals() {
	...
	for (date in valueForDay) {
		...
		valueForDay[date].total =
			valueForDay[date].perStop + valueForDay[date].gasDiem + valueForDay[date].routePay;
...
function drawFields() {
	...
	for (let days = 0; days < 40; days++) {
		...
		for (route in activeRoutes) {
			if (activeRoutes[route] === true && document.getElementById(`${route}Container`) == null) {
			...
			if (
				...
				document.getElementById(`${route}Container`).innerHTML += createRouteDayField(
					dateInfo[1],
					dateInfo[2],
					route
				);
			}
			if (document.getElementById(`${dateInfo[1]}${dateInfo[2]}Total`) == null) {
				totalsContainerElement.innerHTML += createTotalsField(dateInfo[1], dateInfo[2], 0);
			}
		}
...
for (route in routes) {
	...
	routesElement.innerHTML = `${routesElement.innerHTML}${createRouteCheckBox(route)}`;
}
